  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      if (distance >= radius) {
        ...
        letterRef.style.fontWeight = '700';
        return;
      }
      ...
      // Add scale effect for visual feedback
      const scale = 1 + (falloffValue * 0.15);
      letterRef.style.transform = `scale(${scale})`;
      
      // Interpolate font weight for regular fonts
      const weight = 700 + Math.floor(falloffValue * 200);
      letterRef.style.fontWeight = weight.toString();
    });
  });
